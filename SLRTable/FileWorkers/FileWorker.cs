...
public static class FileWorker
{
    ...
    public static void ValidateRules(string[] rawRules)
    {
        ...
        if (unusedRules.Any())
        {
            throw new Exception($"Обнаружено неиспользуемое правило:\n{String.Join("\n", unusedRules)}");
        }
    }
    ...
    public static void ValidateProductivity(Dictionary<string, List<List<string>>> rulesDict)
    {
        ...
        while (updated)
        {
            ...
            foreach (var rule in rulesDict)
            {
                ...
                if (rule.Value.Any(variant =>
                        variant.All(symbol => productive.Contains(symbol) || !rulesDict.ContainsKey(symbol))))
                {
        ...
        {
            throw new Exception($"Обнаружены непродуктивные правила:\n{String.Join("\n", unproductiveRules)}");
        }
    }
}